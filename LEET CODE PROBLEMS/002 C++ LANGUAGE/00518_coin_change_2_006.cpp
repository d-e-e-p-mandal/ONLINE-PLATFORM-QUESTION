...
public:
    int change(int amount, vector<int>& coins) {
        ...
        int n = coins.size();
        vector<long> prev(amount+1, 0), curr(amount+1,0); // dp
        ...
        for(int i =1; i<n; i++) { // index
            for(int j=0; j<= amount; j++) { // amount
                ...
                int pick = 0;
                if(coins[i] <= j) pick = curr[j-coins[i]];  // we take it form same index and previos value in unchanged so we can do it using 1-D array
                curr[j] = (long) notPick + pick; // type casting : (total ways)
            }
            prev = curr;
        }
