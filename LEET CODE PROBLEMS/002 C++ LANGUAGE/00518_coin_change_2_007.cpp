...
public:
    int change(int amount, vector<int>& coins) {
        ...
        int n = coins.size(); 
        vector<long>prev(amount+1); // data structure
        ...
        for(int i=1; i<=n; i++) { // index taking as +1 for handle ostside -1 index
            for(int j=0; j<= amount; j++) {  // amount
                ...
                int pick = 0; 
                if(coins[i-1] <= j) pick = prev[j-coins[i-1]]; // pick : coins inde 0 to n-1 : previous valuse is unchanged so we can use 1-D array

                prev[j] = (long) notPick + pick; // total ways
            }
        }
