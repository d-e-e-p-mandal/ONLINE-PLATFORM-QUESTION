...
public:
    int cherryPickup(vector<vector<int>>& grid) {
        ...
        //dp to store the all value : bottom up
        vector<vector<vector<int>>>dp(r,vector<vector<int>>(c,vector<int>(c,-1)));

        // base casse : each cell of last row (bottom up approach)
        ...
        for(int row=r-2; row>=0; row--){
            for(int c1=0; c1<c; c1++){
                for(int c2=0; c2<c; c2++){
                    int curr;
                    // pick up on its index
                    if(c1 == c2) curr = grid[row][c1]; 
                    ...
                    for(int i=-1; i <= +1; i++){
                        for(int j=-1; j<= +1 ; j++){
                            int prev = ((c1+i >= 0 && c1+i < c && c2+j >= 0 && c2+j < c ) ? dp[row +1][c1+i][c2+j] : -1e8);
                            maxi = max(maxi,prev); //taking maximum
                        }
                    }
                    dp[row][c1][c2] = curr + maxi; //stored in dp
