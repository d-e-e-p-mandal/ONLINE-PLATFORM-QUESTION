...
public:
    int longestCommonSubsequence(string text1, string text2) {
        ...
        vector<int> prev(n+1,0), curr(n+1, 0); //dp
        ...
        for(int i = 1; i<=m; i++) {
            for(int j=1; j<=n; j++) {
                if(text1[i-1] == text2[j-1]) curr[j] = 1 + prev[j-1]; // if both element are same
                else curr[j] = max(prev[j], curr[j-1]); // if dp[i-1] means previous line and dp[i] means current line
            }
            prev = curr; // 
        }
