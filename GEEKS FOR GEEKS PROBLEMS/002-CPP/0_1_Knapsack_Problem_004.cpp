// GFG : 0 - 1 Knapsack Problem : Dynamic Programming (Tabulation) : Space Optimized (extreme)
#include <bits/stdc++.h>
using namespace std;
...
class Solution {
  public:
    int knapsack(int W, vector<int> &val, vector<int> &wt) {
        ...
        for(int i=1; i<n; i++){
            // we using prev[j-wt[i]] it means it is dependent only with left side of j of prev, because substract wt[index] from j. (we take it from prev) so we iterate j from W to 0
            for(int j=W; j>=0; j--){
