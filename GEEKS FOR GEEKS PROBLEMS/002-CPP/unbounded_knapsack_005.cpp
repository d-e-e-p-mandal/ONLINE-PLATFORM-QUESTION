...
class Solution {
  public:
    int knapSack(vector<int>& val, vector<int>& wt, int capacity) {
        ...
        int n = val.size();
        vector<int> prev(capacity+1, 0), curr(capacity+1); // dp
        ...
        for(int i=1; i<=n; i++) {  // index start at 1 : index 0 in base case (index -1 is represented as 0 : using +1 with all index)
            for(int j=0; j<=capacity; j++) { // capacity
                ...
                int pick = 0;
                if(wt[i-1] <= j) pick = val[i-1] + curr[j-wt[i-1]]; // pick index : wt and val index 0 to n-1
                
                curr[j] = max(notPick, pick); // maximum of both
            }
            prev = curr;
        }
...
/*
	...
	Space Complexity: O(2* capacity) → O(2* capacity) for DP array.
*/